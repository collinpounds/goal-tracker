...
import type { RootState, AppDispatch, TeamCreate, Team } from '../types';
...
interface TeamFormData extends TeamCreate {
  parent_team_id: number | null;
}

interface TeamWithNesting extends Team {
...
const TeamFormModal = () => {
  ...
  const { showTeamForm, editingTeam, teams, loading, error } = useSelector((state: RootState) => state.teams);
  ...
  useEffect(() => {
    const typedEditingTeam = editingTeam as TeamWithNesting | null;
    if (typedEditingTeam) {
      setFormData({
        name: typedEditingTeam.name || '',
        description: typedEditingTeam.description || '',
        parent_team_id: typedEditingTeam.parent_team_id || null,
      });
    } else {
      setFormData({
        name: '',
  ...
  const teamsWithNesting = teams as TeamWithNesting[];
  const availableParentTeams = teamsWithNesting.filter((team) => {
    const typedEditingTeam = editingTeam as TeamWithNesting | null;
    if (!typedEditingTeam) return (team.nesting_level || 0) < 2; // Can only nest up to level 2
    return team.id !== typedEditingTeam.id && (team.nesting_level || 0) < 2;
  });
