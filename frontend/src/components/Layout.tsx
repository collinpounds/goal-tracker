...
const Layout = () => {
  ...
  const { showForm, editingGoal } = useSelector((state: RootState) => state.goals);
  ...
  const handleCreateGoal = async (goalData: GoalCreate | GoalUpdate) => {
    const { team_ids, category_ids, ...goalDataWithoutRelations } = goalData;

    const result = await dispatch(createGoal(goalDataWithoutRelations as GoalCreate));

    if (result.payload) {
      const goalId = (result.payload as { id: number }).id;
  ...
  const handleUpdateGoal = async (goalData: GoalCreate | GoalUpdate) => {
    if (!editingGoal) return;

    const { team_ids, category_ids, ...goalDataWithoutRelations } = goalData;

    const result = await dispatch(updateGoal({ id: editingGoal.id, goalData: goalDataWithoutRelations as GoalUpdate }));
